...
//				 They can also be compared with == or !=. The = 
//				 operator can be used to copy autobots. Lastly, the
...
//				 10/15/2017 MN completed version 1.1
//
//----------------------------------------------------------------------------
	...
	DEFAULT_WEIGHT_IN_KILOGRAMS(2000.0), DEFAULT_POWER_LEVEL(5)
{
	...
	BotHasher myHasher = BotHasher(*this);
}
...
//----------------------------------------------------------------------------
// Copies the data from one autobot to another.
//----------------------------------------------------------------------------

Autobot & Autobot::operator=(const Autobot & autobot)
{
	name = autobot.getName();
	heightInCentimeters = autobot.getHeightInCentimeters();
	weightInKilograms = autobot.getWeightInKilograms();
	powerLevel = autobot.getPowerLevel();
	numKills = autobot.getNumKills();
	alive = autobot.getIsAlive();
	return *this;
}
